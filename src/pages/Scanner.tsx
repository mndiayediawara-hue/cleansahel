...
import { ScanLine, Camera, CheckCircle2, AlertCircle, Package, Boxes, Beaker, Users, X, Zap, Hash, Layers } from 'lucide-react'
...
export default function Scanner() {
  ...
  const typeMeta: Record<string, { icon: any; label: string; color: string }> = {
    ...
    lote_mp: { icon: Beaker, label: 'Lote de Materia Prima', color: 'bg-amber-100 text-amber-700' },
    lote_producto: { icon: Layers, label: 'Lote de Producto', color: 'bg-emerald-100 text-emerald-700' },
  }
                ...
                return (
                  <div className="space-y-2">
                    ...
                    <div className="p-4 rounded-lg bg-surface-50 dark:bg-surface-800/50 space-y-1.5 text-sm">
                      ...
                      {result.type === 'lote_mp' && (
                        <>
                          {d.supplierLotNumber && <div className="flex justify-between"><span className="text-surface-500">Lote proveedor:</span><span className="font-mono">{d.supplierLotNumber}</span></div>}
                          {d.expiryDate && <div className="flex justify-between"><span className="text-surface-500">Caducidad:</span><span className="font-semibold">{new Date(d.expiryDate).toLocaleDateString('es-ES')}</span></div>}
                          {d.status && <div className="flex justify-between"><span className="text-surface-500">Estado:</span><span className={`font-semibold ${d.status === 'activo' ? 'text-emerald-600' : 'text-amber-600'}`}>{d.status}</span></div>}
                        </>
                      )}
                    </div>
                  </div>
